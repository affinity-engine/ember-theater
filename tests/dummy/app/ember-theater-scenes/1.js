...
export default Scene.extend({
  script: async function(director) {
    // await director.choice({ header: '1.choice1.header', choices: { A: { text: { id: '1.choice1.A', options: { opt1: 'OOO' } } }, B: '1.choice1.B' } });
    // director.filter({ effect: 'blur(2px)', options: { duration: 500 }, layer: 'theater' });
    // await director.dialogue({ character: 'steven', text: '1.noId' });
    // director.filter({ effect: 'blur(0px)', options: { duration: 500 }, layer: 'theater' });
    // await director.dialogue({ character: 'steven', displayName: '1.noId', text: { id: '1.firstTest', options: { val1: 'my friend' } } });
    ...
    director.setStat('test', 0);
    director.transitionToScene(2);
  }
});
